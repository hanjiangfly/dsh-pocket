...
export function createPocketService({
  ...
  injectHtml,
  /** 每次 HTML 响应按访客身份追加的轻量上下文（仅 UI 隔离）。 */
  visitorContextScript,
  ...
  /** 自动恢复的目标模式。与用户开关同源持久化，避免单独状态文件丢失。 */
  getPublicAutoRestoreMode = () => null,
  setPublicAutoRestoreMode = () => null,
  ...
  async function persistAutoTunnel(mode = 'quick', hostname = '') {
    // settings.json 是开关本身的存储位置；优先把目标写在这里，重启后不会出现
    // “开关仍是开，但 tunnel-auto.json 不在当前 DSH_HOME” 的不同步状态。
    try { setPublicAutoRestoreMode(mode); } catch { /* 忽略 */ }
  ...
  async function clearAutoTunnel() {
    try { setPublicAutoRestoreMode(null); } catch { /* 忽略 */ }
    if (!autoStatePath) return;
  ...
  return {
      ...
      for (let p = startPort; p < endPort; p++) {
        try {
          proxy = await createProxy({
            ...
            ...(visitorContextScript ? { visitorContextScript } : {}),
            ...(auth ? { auth } : {}),
    ...
    async restoreTunnelIfNeeded() {
      if (!getPublicAutoRestore() || tunnel || tunnelPromise) return;
      // 新版本先读 settings.json 中的目标；它与“自动恢复”开关同源持久化。
      // 旧版本只留下开关时，已完成初始化的固定域名可安全推断为 fixed；否则兼容 quick。
      let mode = getPublicAutoRestoreMode?.();
      if (mode !== 'quick' && mode !== 'fixed') {
        try {
          const raw = await readFile(autoStatePath, 'utf8');
          const parsed = JSON.parse(raw);
          mode = parsed?.mode;
        } catch { /* 旧状态文件不存在时继续做兼容推断 */ }
      }
      if (mode !== 'quick' && mode !== 'fixed') {
        mode = (getFixedHostname?.() && getFixedTunnelId?.()) ? 'fixed' : 'quick';
      }
      try {
