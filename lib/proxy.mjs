 ...
 */
export function createPocketProxy({ port = 3081, host = '0.0.0.0', upstream = DEFAULT_UPSTREAM, log = null, injectHtml = DEFAULT_INJECT, visitorContextScript = null, auth = null, rateLimit = null, heartbeat = {}, lanAccessEnabled = () => true, isPublicHost = defaultIsPublicHost } = {}) {
  const limiter = auth ? createRateLimiter(rateLimit ?? {}) : null;
  const server = createServer((req, res) => {
    const host = String(req.headers.host ?? '');
    const isPublic = isPublicHost(host);
    // 仅用于页面 UI 隔离的身份提示；真正的代理认证仍由 HttpOnly Cookie 完成。
    let visitorIdentity = { kind: 'owner' };
    ...
    if (auth) {
      ...
      if (protectedHost && token) {
        ...
        visitorIdentity = authIdentity(req, host, token, sessionKey, auth.guestAccess);
        if (!visitorIdentity) {
          if (isHtmlRequest(req)) {
        ...
        if (injectHtml && contentType.includes('text/html') && !isCompressed(proxyRes.headers)) {
          ...
          proxyRes.on('end', () => {
            ...
            if (!html.includes(INJECT_MARK)) {
              const visitorScript = visitorContextScript?.(visitorIdentity) ?? '';
              html = html.replace(/<head[^>]*>/i, (m) => `${m}${injectHtml}${visitorScript}`);
            }
