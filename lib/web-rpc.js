...
export function installPocketRpc(ctx, { service, log = console, desktop = false, runUpdate = null, restart = null, restartNotice = null, getToken = null, getLanToken = null, refreshLanToken = null, getLanAuthEnabled = null, setLanAuthEnabled = null, getLanEnabled = null, setLanEnabled = null, getLanIpOverride = null, setLanIpOverride = null, setProxyPort = null, getPinCustom = null, setCustomPin = null, getFixedHostname = null, setFixedHostname = null, getFixedAccessEnabled = null, setFixedAccessEnabled = null, getFixedPinAlways = null, setFixedPinAlways = null, fixedSetup = null }) {
  ...
  return ctx.connection.rpc.handle(POCKET_RPC_CHANNEL, async (endpoint, payload = {}, signal) => {
    ...
    try {
      ...
      if (endpoint === POCKET_ENDPOINTS.proxySetPort) {
        try {
          const result = await service.restartProxyOnPort?.(payload?.port ?? '');
          if (!result) return fail('bad-request', '代理端口设置不可用 | proxy port setting unavailable');
          setProxyPort?.(result.custom ? result.port : '');
          return await statusPayload();
        } catch (err) {
