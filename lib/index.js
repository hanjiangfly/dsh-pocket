...
import { lanEnabled, setLanEnabled, lanAuthEnabled, setLanAuthEnabled, lanIpOverride, setLanIpOverride, pinCustom, setPinCustom, fixedHostname, setFixedHostname, fixedTunnelId, setFixedTunnelId, fixedRouted, setFixedRouted, fixedAccessEnabled, setFixedAccessEnabled, fixedPinAlways, setFixedPinAlways, publicAutoRestore, setPublicAutoRestore, publicAutoRestoreMode, setPublicAutoRestoreMode } from './settings.mjs';
import { resolveCloudflared, ensureNamedTunnel, routeDns, certExists } from './tunnel.mjs';
import { createGuestAccessManager } from './guest-access.mjs';
...
/** 按 Host 分发的访问密码：公网（快速隧道/固定域名）用公网密码，局域网用局域网密码。 */
function tokenForHost(host) {
  return isPublicHost(host) ? getAccessToken() : getLanToken();
}

// ---------- 固定域名（命名隧道）认证策略 ----------
// 固定域名 URL 固定不再轮换，认证层建议 Cloudflare Access（边缘 MFA）。
// PIN 策略（见 settings.mjs 头注释）：
//   - 快速隧道（trycloudflare）永远要 PIN；
//   - 固定域名：未启用 Access → 强制 PIN（裸奔不允许）；启用 Access → 默认免 PIN，
//     用户可选「额外要求 PIN」做纵深防御（双因素）。
// 固定域名也视为「公网 Host」：关闭局域网访问时不拦截、登录页文案用公网措辞。

/** 该 Host 是否公网：快速隧道（trycloudflare）或固定域名（精确匹配，忽略端口）。 */
export function isPublicHost(host) {
  const h = String(host ?? '').trim().toLowerCase();
  if (/trycloudflare\.com$/i.test(h)) return true;
  const fh = fixedHostname();
  return Boolean(fh && h.split(':')[0] === fh);
}

/** 固定域名一次性初始化：登录完成后，自动建隧道 + 绑 DNS（幂等）。 */
async function fixedDomainSetup() {
  const hostname = fixedHostname();
  if (!hostname) throw new Error('请先在「固定域名」区块填写域名 | set a hostname in the fixed-domain section first');
  if (!(await certExists())) {
    throw new Error('请先完成第 1 步：点「登录授权」并在浏览器里完成 Cloudflare 授权 | finish step 1: log in to Cloudflare in your browser first');
  }
  const bin = await resolveCloudflared({ signal: AbortSignal.timeout(180_000) });
  const { id, existing } = await ensureNamedTunnel({ bin, name: 'dsh-pocket' });
  setFixedTunnelId(id);
  await routeDns({ bin, name: 'dsh-pocket', hostname });
  setFixedRouted(true);
  return { id, hostname, existing: existing === true };
}
/**
...
export function apply(ctx, config = {}, internals = {}) {
  ...
  // 只有 service 的匿名边缘探测可置真；进程重启和 Access 开关变动都会回到 false。
  let fixedAccessVerified = false;
  // service 的隧道恢复标记必须和 settings/token 使用同一个 DSH 数据目录。
  // internals.home 仅用于测试注入；生产环境不能把 undefined 传进去，否则不会创建 tunnel-auto.json。
  const pocketHome = internals.home ?? process.env.DSH_HOME ?? join(homedir(), '.dsh');
  const guestAccess = internals.guestAccess ?? createGuestAccessManager({
    path: internals.guestAccessPath,
    isPublicHost,
  });
  const service = internals.service ?? createPocketService({
    ...
    // 固定域名（命名隧道）配置：设置页向导写入，代理按此决定 PIN 策略与公网判定
    getFixedHostname: () => fixedHostname(),
    getFixedTunnelId: () => fixedTunnelId(),
    getFixedRouted: () => fixedRouted(),
    getFixedAccessEnabled: () => fixedAccessEnabled(),
    getFixedPinAlways: () => fixedPinAlways(),
    getPublicAutoRestore: () => publicAutoRestore(),
    getPublicAutoRestoreMode: () => publicAutoRestoreMode(),
    setPublicAutoRestoreMode: (mode) => setPublicAutoRestoreMode(mode),
    setFixedAccessVerified: (verified) => { fixedAccessVerified = verified; },
    isPublicHost,
    ...
    injectHtml: isDesktop
      ...
      : undefined,
    // 访客 Cookie 只能由代理读取；将不可敏感的 role 注入页面，仅用于插件设置页只读 UI。
    // 注意：这不是授权边界，管理 RPC 的强制鉴权将在后续的连接身份方案中实现。
    visitorContextScript: (identity) => identity?.kind === 'guest'
      ? '<script>window.__dshPocketVisitor={role:"guest"};</script>'
      : '<script>window.__dshPocketVisitor={role:"owner"};</script>',
    // 访问密码（issue #13 + #18 + #24 + #33）：快速隧道永远要密码（默认每次开启变新，
    // 用户自定义后不再轮换）；固定域名按 Access 开关（未启用强制 PIN；启用默认免 PIN，
    // 可选额外要求）；局域网按开关（默认开启；关闭后局域网扫码直连）。
    auth: {
      ...
      isProtected: (host) => {
        if (/trycloudflare\.com$/i.test(String(host ?? ''))) return true; // 快速隧道永远要密码
        if (isPublicHost(host)) {
          // 固定域名：Access 未验证或未启用 → 强制 PIN；验证通过后才允许按额外 PIN 开关决定。
          return !fixedAccessEnabled() || !fixedAccessVerified || fixedPinAlways();
        }
        return lanAuthEnabled();
      },
    },
  ...
  const disposeRpc = installPocketRpc(ctx, {
    ...
    // 固定域名：设置页读写 + 一次性初始化（登录 → 建隧道 → 绑 DNS）
    getFixedHostname: () => fixedHostname(),
    setFixedHostname: (v) => setFixedHostname(v),
    getFixedAccessEnabled: () => fixedAccessEnabled(),
    setFixedAccessEnabled: (on) => {
      const value = setFixedAccessEnabled(on);
      fixedAccessVerified = false;
      service.resetFixedAccessVerification?.();
      return value;
    },
    getFixedPinAlways: () => fixedPinAlways(),
    setFixedPinAlways: (on) => setFixedPinAlways(on),
    getPublicAutoRestore: () => publicAutoRestore(),
    setPublicAutoRestore: async (on) => {
      const value = setPublicAutoRestore(on);
      await service.setAutoRestoreEnabled?.(value);
      return value;
    },
    fixedSetup: fixedDomainSetup,
    guestAccess,
