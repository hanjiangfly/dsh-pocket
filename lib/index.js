...
import { lanEnabled, setLanEnabled, lanAuthEnabled, setLanAuthEnabled, lanIpOverride, setLanIpOverride, proxyPort, setProxyPort, pinCustom, setPinCustom, fixedHostname, setFixedHostname, fixedTunnelId, setFixedTunnelId, fixedRouted, setFixedRouted, fixedAccessEnabled, setFixedAccessEnabled, fixedPinAlways, setFixedPinAlways } from './settings.mjs';
...
export function apply(ctx, config = {}, internals = {}) {
  ...
  const savedProxyPort = proxyPort();
  const service = internals.service ?? createPocketService({
    dshPort,
    port: internals.port ?? savedProxyPort ?? config.port ?? 3081,
    strictPort: savedProxyPort !== null,
    home: internals.home,
    internals,
  ...
  const disposeRpc = installPocketRpc(ctx, {
    ...
    setProxyPort: (value) => setProxyPort(value),
