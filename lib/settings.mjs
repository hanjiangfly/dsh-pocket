...
//   - publicAutoRestore   是否允许 DSH 重启后自动恢复公网访问（默认关，需用户确认风险后开启）
//   - publicAutoRestoreMode 自动恢复的目标（quick | fixed）；与开关保存在同一文件，避免状态不同步
...
/** 是否已明确允许 DSH 重启后恢复公网隧道；默认关闭，避免意外重新暴露公网。 */
export function publicAutoRestore() {
  return readSettings().publicAutoRestore === true;
}

/** 设置公网自动恢复授权。关闭后下次重启绝不自动恢复。 */
export function setPublicAutoRestore(on) {
  const s = readSettings();
  s.publicAutoRestore = !!on;
  if (!s.publicAutoRestore) delete s.publicAutoRestoreMode;
  writeSettings(s);
  return s.publicAutoRestore;
}

/** 自动恢复目标；null 表示旧版本留下的开关状态、尚未记录具体模式。 */
export function publicAutoRestoreMode() {
  const mode = readSettings().publicAutoRestoreMode;
  return mode === 'quick' || mode === 'fixed' ? mode : null;
}

/** 记录自动恢复的公网方式，和开关写入同一份 settings.json。 */
export function setPublicAutoRestoreMode(mode) {
  const s = readSettings();
  if (mode === 'quick' || mode === 'fixed') s.publicAutoRestoreMode = mode;
  else delete s.publicAutoRestoreMode;
  writeSettings(s);
  return publicAutoRestoreMode();
}
